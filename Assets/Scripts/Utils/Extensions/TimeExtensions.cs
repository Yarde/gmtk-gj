...
namespace Yarde.Utils.Extensions
{
    public static class DateTimeExtensions
    {
        ...
        public static DateTime ToDateTimeFromMilliseconds(this long timestampInMs) =>
            Epoch.AddMilliseconds(timestampInMs);

        public static DateTime ToDateTimeFromSeconds(this long timeStampInSeconds) =>
            Epoch.AddSeconds(timeStampInSeconds);

        public static DateTime ToDateTimeFromMilliseconds(this int timestampInMs) =>
            Epoch.AddMilliseconds(timestampInMs);

        public static DateTime ToDateTimeFromSeconds(this int timeStampInSeconds) =>
            Epoch.AddSeconds(timeStampInSeconds);

        public static string GetLocalScheduleTime(this DateTime dateTime) =>
            dateTime.ToLocalTime().ToString(SCHEDULE_TIME_FORMAT);

        public static bool IsToday(this DateTime nextMatchTime) => nextMatchTime.Date == DateTime.Today;

        public static bool IsTomorrow(this DateTime nextMatchTime) =>
            nextMatchTime.Date - TimeSpan.FromDays(1) == DateTime.Today;
        ...
        public static async UniTask WaitForDelayInRealTime(long milliseconds,
            CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            {
                await UniTask.Delay((int)Math.Min(milliseconds, 1000), DelayType.Realtime,
